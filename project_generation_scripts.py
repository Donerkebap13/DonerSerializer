...
THIRDPARTY_PATH="{donerreflection_folder_path}"
...
set donerreflection_folder="{donerreflection_folder_path}"
...
IF EXIST %project_folder% (
    ...
    rmdir /s /q %donerreflection_folder%
)
...
def generate_scripts_for_platform(platform, generate_tests, xcode):
    ...
    donerreflection_folder_path = path_to_os("{}/DonerSerializer/thirdparty/DonerReflection".format(app_folder))

    create_folder_if_not_exists(script_folder)

    cmake_call_debug = generate_cmake_call("Debug", platform, generate_tests, xcode)
    cmake_call_release = generate_cmake_call("Release", platform, generate_tests, xcode)

    generate_templated_file(generate_script_file_debug, generate_template, project_folder_path_debug, donerreflection_folder_path, cmake_call_debug)
    generate_templated_file(generate_script_file_release, generate_template, project_folder_path_release, donerreflection_folder_path, cmake_call_release)

    generate_templated_file(update_script_file_debug, update_template, project_folder_path_debug, donerreflection_folder_path, cmake_call_debug)
    generate_templated_file(update_script_file_release, update_template, project_folder_path_release, donerreflection_folder_path, cmake_call_release)


def generate_cmake_call(configuration, platform, generate_tests, xcode):
    tests_flags = ""
    if(generate_tests):
        tests_flags = "-DDONER_ENABLE_TESTS=1"
    ...
    root_cmake_path = path_to_os("{}/DonerSerializer".format(app_folder))

    return 'cmake "{}" -DCMAKE_CONFIGURATION_TYPES="{}" {} {} {}'.format(root_cmake_path, configuration, common_cmake_flags, platform_flags, tests_flags).replace("  ", " ").replace("  ", " ")


def generate_templated_file(script_file_path, template, project_folder_path, donerreflection_folder_path, cmake_call):
    with open(script_file_path, "w") as file:
        file.write(template.format(
            project_folder_path=project_folder_path,
            donerreflection_folder_path=donerreflection_folder_path,
...
def read_parameters():
    ...
    return args.generate_tests, args.all_platforms, args.xcode


def generate_scripts(generate_tests, all_platforms, xcode):
    if all_platforms:
        for platform in VALID_PLATFORMS:
            generate_scripts_for_platform(platform, generate_tests, xcode)
    elif sys.platform in VALID_PLATFORMS:
        generate_scripts_for_platform(sys.platform, generate_tests, xcode)
    else:
        message_and_die("You're working under an unsupported OS! Valid OS are {}".format(VALID_PLATFORMS))


if __name__ == '__main__':
    generate_tests, all_platforms, xcode = read_parameters()
    generate_scripts(generate_tests, all_platforms, xcode)
